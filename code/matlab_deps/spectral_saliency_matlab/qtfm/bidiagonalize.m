...
if r == 1 || c == 1
    R = s(B); % The diagonal has only one element, so we can just forget
              % the off-diagonal.
else
    R = s(B - O); % Subtract the off-diagonal part and take the scalar part
                  % of the result.
end


% $Id: bidiagonalize.m,v 1.7 2009/02/08 19:18:16 sangwine Exp $
